class MinHeap:
    ...
    def heapifyDown(self):
        ...
        while self.__hasLeftChild(i):
            smallerChildIndex = self.__getLeftChildIndex(i)
            if self.__hasRightChild(i) and\
                    self.heapList[self.__getRightChildIndex(i)] < self.heapList[smallerChildIndex]:
                smallerChildIndex = self.__getRightChildIndex(i)
            if self.heapList[i] < self.heapList[smallerChildIndex]:
                break
            else:
                self.heapList[i], self.heapList[smallerChildIndex] =\
                    self.heapList[smallerChildIndex], self.heapList[i]
            i = smallerChildIndex
